...
?>
<head>
    <link rel="stylesheet" href="https://maxcdn.bootstrapcdn.com/bootstrap/4.0.0-alpha.6/css/bootstrap.min.css" integrity="sha384-rwoIResjU2yc3z8GV/NPeZWAv56rSmLldC3R/AZzGRnGxQQKnKkoFVhFQhNUwEyJ" crossorigin="anonymous">    <link rel="stylesheet" href="http://ekanzler.greenriverdev.com/328/dating/styles/styles.css">
    <link rel="stylesheet" href="http://ekanzler.greenriverdev.com/328/dating/styles/styles.css">
...
$f3->route('POST /submit-profile', function($f3) {
    ...
    // /dating/tmp/ekz1lda5ui63.19jl7d1ux5i3m.php:12
    ...
    if(isset($_FILES['profilePic'])) {
        ...
        if($member->getProfilePic() != null) {
            ...
            while(file_exists("images/".$name.$extension)){
                $name = $f3->get('SESSION.member')->getFname().
                    "-".$f3->get('SESSION.member')->getLname()."(".$index.")";
                $index ++;
            }
...
$f3->route('GET /view-@id', function($f3) {
    $template = new Template;
    $dbh = $f3->get('pdo');
    ...
    $statement->execute();
    $f3->set('viewMember', $statement->fetch(PDO::FETCH_ASSOC));
